...
layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Dropdown(
            id='month',
            options=[{'label': i, 'value': i} for i in list_month].sort(),
            value=list_month[-1]
        )
    ]),
...
def update_figure(year, month):
    ...
    if((dftime['pm10value'].max()-dftime['pm10value'].min())>10000):
        round_down=-4
    ...
    fig.update_xaxes(range=[round(dftime['pm10value'].min(),round_down),round(dftime['pm10value'].max(),round_down)])
